<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Modifier Group') }} — {{ $group->name }}
        </h2>
    </x-slot>

    <div class="max-w-3xl">
        <form method="POST" action="{{ route('modifier-groups.update', $group) }}" class="space-y-6"
              data-draft-key="modifier-group-edit-{{ $group->id }}"
              x-data="{
                  options: @js($group->options->map(fn ($option) => [
                      'id' => $option->id,
                      'name' => $option->name,
                      'price_delta' => (float) $option->price_delta,
                      'sku' => $option->sku,
                  ])->values()->all() ?: [['id' => null, 'name' => '', 'price_delta' => '', 'sku' => '']]),
                  addOption() { this.options.push({ id: null, name: '', price_delta: '', sku: '' }); },
                  removeOption(index) { this.options.splice(index, 1); },
              }"
              x-persist="{ key: 'modifier-group-edit-{{ $group->id }}-options', paths: ['options'] }"
        >
            @csrf
            @method('PUT')
            ...
            <x-forms.section :title="__('Group Details')">
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="block mt-1 w-full" :value="old('name', $group->name)" required autofocus />
                ...
                <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select id="selection_type" name="selection_type" class="block mt-1 w-full border-gray-300 focus:border-[#8A3330] focus:ring-[#8A3330] rounded-md shadow-sm">
                            @foreach ($selectionTypes as $type)
                                <option value="{{ $type->value }}" @selected(old('selection_type', $group->selection_type->value) === $type->value)>{{ $type->label() }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="sort_order" name="sort_order" type="number" min="0" class="block mt-1 w-full" :value="old('sort_order', $group->sort_order)" />
                ...
                <div class="mt-5 flex items-center">
                    <input id="is_required" name="is_required" type="checkbox" value="1"
                           class="rounded border-gray-300 text-[#8A3330] shadow-sm focus:ring-[#8A3330]" @checked(old('is_required', $group->is_required))>
