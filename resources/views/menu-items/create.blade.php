<x-app-layout>
    ...
    @php
        // A category is always pre-selected (the first one) rather than a
        // blank placeholder, so Sort Order can show a real number the
        // moment the page loads instead of sitting blank until the user
        // touches the dropdown.
        $defaultCategoryId = old('menu_category_id', optional($categories->first())->id);
        $defaultSortOrder = old('sort_order') !== null
            ? (int) old('sort_order')
            : ($defaultCategoryId && isset($nextSortOrders[$defaultCategoryId]) ? $nextSortOrders[$defaultCategoryId] : 0);
    @endphp

    <div>
        <form method="POST" action="{{ route('menu-items.store') }}" enctype="multipart/form-data"
              data-draft-key="menu-item-create"
              x-data="{
                  nextSortOrders: @js($nextSortOrders),
                  sortOrder: {{ $defaultSortOrder }},
                  sortOrderTouched: {{ old('sort_order') !== null ? 'true' : 'false' }},
                  applyNextSortOrder(categoryId) {
                      if (! this.sortOrderTouched) {
                          this.sortOrder = this.nextSortOrders[categoryId] ?? 0;
                      }
                  },
              }"
        >
                ...
                <div class="lg:col-span-2 space-y-6">
                    <x-forms.section :title="__('Basic Information')">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <select id="menu_category_id" name="menu_category_id" class="block mt-1 w-full border-gray-300 focus:border-[#8A3330] focus:ring-[#8A3330] rounded-md shadow-sm" required
                                        @change="applyNextSortOrder($event.target.value)">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" @selected($defaultCategoryId == $category->id)>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mt-5">
                            ...
                            <textarea id="description" name="description" rows="2"
                                      class="block mt-1 w-full border-gray-300 focus:border-[#8A3330] focus:ring-[#8A3330] rounded-md shadow-sm text-sm">{{ old('description') }}</textarea>
                    ...
                    <x-forms.section :title="__('Pricing & Prep')">
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                            ...
                            <div>
                                ...
                                <x-text-input id="sku" name="sku" type="text" class="block mt-1 w-full" :value="old('sku')" />
                                <x-input-error :messages="$errors->get('sku')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="prep_time_minutes" :value="__('Prep Time (min)')" />
                                <x-text-input id="prep_time_minutes" name="prep_time_minutes" type="number" min="0" class="block mt-1 w-full" :value="old('prep_time_minutes')" />
                    ...
                    <x-forms.section :title="__('Variants')" :description="__('Optional — use instead of creating duplicate items for things like Solo/Medium/Large, Spicy/Oriental, or Half/Whole. Leave empty if this item is just sold at the Price above.')">
                        <div x-data="{
                                ...
                                defaultIndex: null,
                                addVariant() {
                                    this.variants.push({ id: null, name: '', sku: '', price: '', imagePreview: null });
                             ...
                             x-persist="{ key: 'menu-item-create-variants', paths: ['variants', 'defaultIndex'] }"
                             data-draft-ignore
                        >
                            <template x-for="(variant, index) in variants" :key="index">
                                <div class="flex gap-3 border border-[#E5DDD0] rounded-lg p-3 mb-3">
                                    ...
                                    <label class="shrink-0 relative h-16 w-16 rounded-md bg-[#FAF6EE] border border-dashed border-[#D9CCBA] overflow-hidden cursor-pointer flex items-center justify-center hover:border-[#8A3330]">
                                        <img x-show="variant.imagePreview" :src="variant.imagePreview" class="absolute inset-0 h-full w-full object-cover">
                                        <span x-show="!variant.imagePreview" class="text-[9px] text-gray-400 text-center px-1 leading-tight">{{ __('Photo (optional)') }}</span>
                                        <input type="file" accept="image/*" :name="'variants[' + index + '][image]'" class="sr-only"
                                               @change="variant.imagePreview = $event.target.files[0] ? URL.createObjectURL($event.target.files[0]) : variant.imagePreview">
                                    </label>
                    ...
                    <x-forms.section :title="__('Availability & Flags')">
                        <div>
                            ...
                            <select id="availability_status" name="availability_status" class="block mt-1 w-full border-gray-300 focus:border-[#8A3330] focus:ring-[#8A3330] rounded-md shadow-sm">
                                @foreach ($availabilityOptions as $option)
                                    <option value="{{ $option->value }}" @selected(old('availability_status', 'available') === $option->value)>{{ $option->label() }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-5">
                            ...
                            <x-text-input id="sort_order" name="sort_order" type="number" min="0" class="block mt-1 w-full"
                                          x-model.number="sortOrder" @input="sortOrderTouched = true" />
                            <p class="mt-1 text-xs text-gray-400">{{ __('Auto-filled with the next number in this category.') }}</p>
                        ...
                        <div class="mt-5 flex flex-col gap-3">
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                <input id="is_featured" name="is_featured" type="checkbox" value="1"
                                       class="rounded border-gray-300 text-[#8A3330] shadow-sm focus:ring-[#8A3330]" @checked(old('is_featured'))>
                                <span class="text-sm text-gray-600">{{ __('Featured') }}</span>
                            </label>
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                <input id="is_best_seller" name="is_best_seller" type="checkbox" value="1"
                                       class="rounded border-gray-300 text-[#8A3330] shadow-sm focus:ring-[#8A3330]" @checked(old('is_best_seller'))>
